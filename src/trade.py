...
class BackTest():
    ...
    def adjust_long(self, date, p):
        ...
        self.longBill = self.longBill.append(
            newBill[['price', 'n', 'real', 'date']])
    ...
    def act_start_day(self):
        ...
        tmpLongDay = self.longPosition.index[0]
        if tmpLongDay == self.start:
            self.nextLongIdx = 1
        else:
            self.nextLongIdx = 0
        self.nextLongDay = self.longPosition.index[self.nextLongIdx]
    ...
    def act_adjust_long(self, date):
        """adjust long position

        Args:
            date (date): trading date: '2011-01-04'
        """
        if date == self.nextLongDay:
            # 换多仓
            self.longFlag = True

            # 设置下一个换多头日
            self.nextLongIdx += 1
            if len(self.longPosition) == self.nextLongIdx:  # 避免最后一天bug
                self.nextLongDay = -1
            else:
                self.nextLongDay = self.longPosition.index[self.nextLongIdx]
